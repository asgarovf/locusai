...
3.  **Execution Loop**:
    ...
    *   **Report**: Updates the task status to `IN_REVIEW` (on success) or `BACKLOG` (on failure) with a summary comment.
...
*   **IN_REVIEW**: Agent completed the task and it's awaiting review (manual or reviewer agent).
