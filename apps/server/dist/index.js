...
import { ArtifactSchema, CiRunSchema, CommentSchema, DocWriteSchema, LockSchema, TaskSchema, TaskStatus, TaskUpdateSchema, UnlockSchema, } from "@locus/shared";
...
app.get("/api/tasks", (_req, res) => {
    ...
    const formattedTasks = tasks.map((t) => ({
        ...
        acceptanceChecklist: JSON.parse(t.acceptanceChecklist || "[]"),
    }));
...
app.post("/api/tasks", validate(TaskSchema), (req, res) => {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `)
        .run(title, description, TaskStatus.BACKLOG, JSON.stringify(labels), assigneeRole, "[]", now, now);
    const taskId = result.lastInsertRowid;
    db.prepare("INSERT INTO events (taskId, type, payload, createdAt) VALUES (?, ?, ?, ?)").run(String(taskId), "TASK_CREATED", JSON.stringify({ title }), now);
    res.json({ id: taskId });
});
...
app.patch("/api/tasks/:id", validate(TaskUpdateSchema), (req, res) => {
    ...
    const stmt = db.prepare(`UPDATE tasks SET ${fields.join(", ")}, updatedAt = ? WHERE id = ?`);
    stmt.run(...vals);
    if (updates.status && updates.status !== oldTask.status) {
        db.prepare("INSERT INTO events (taskId, type, payload, createdAt) VALUES (?, ?, ?, ?)").run(id, "STATUS_CHANGED", JSON.stringify({ from: oldTask.status, to: updates.status }), Date.now());
    }
    res.json({ ok: true });
});
app.post("/api/tasks/:id/comment", validate(CommentSchema), (req, res) => {
    ...
    const now = Date.now();
    db.prepare("INSERT INTO comments (taskId, author, text, createdAt) VALUES (?, ?, ?, ?)").run(taskId, author, text, now);
    db.prepare("INSERT INTO events (taskId, type, payload, createdAt) VALUES (?, ?, ?, ?)").run(taskId, "COMMENT_ADDED", JSON.stringify({ author }), now);
    res.json({ ok: true });
});
...
app.post("/api/tasks/:id/lock", validate(LockSchema), (req, res) => {
    ...
    db.prepare("UPDATE tasks SET lockedBy = ?, lockExpiresAt = ?, updatedAt = ? WHERE id = ?").run(agentId, expiresAt, now, id);
    db.prepare("INSERT INTO events (taskId, type, payload, createdAt) VALUES (?, ?, ?, ?)").run(id, "LOCKED", JSON.stringify({ agentId, expiresAt }), now);
    res.json({ ok: true });
});
app.post("/api/tasks/:id/unlock", validate(UnlockSchema), (req, res) => {
    ...
    db.prepare("UPDATE tasks SET lockedBy = NULL, lockExpiresAt = NULL, updatedAt = ? WHERE id = ?").run(now, id);
    db.prepare("INSERT INTO events (taskId, type, payload, createdAt) VALUES (?, ?, ?, ?)").run(id, "UNLOCKED", JSON.stringify({ agentId }), now);
    res.json({ ok: true });
});
// Artifacts
app.post("/api/tasks/:id/artifact", validate(ArtifactSchema), async (req, res) => {
    const { id: taskId } = req.params;
    const { type, title, contentText, fileBase64, fileName, createdBy } = req.body;
    const now = Date.now();
    let filePath = null;
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `)
        .run(taskId, type, title, contentText, filePath, createdBy, now);
    db.prepare("INSERT INTO events (taskId, type, payload, createdAt) VALUES (?, ?, ?, ?)").run(taskId, "ARTIFACT_ADDED", JSON.stringify({ type, title, createdBy }), now);
...
app.post("/api/ci/run", validate(CiRunSchema), async (req, res) => {
    ...
    VALUES (?, ?, ?, ?, ?, ?)
  `).run(taskId, "CI_OUTPUT", `CI Run: ${preset}`, combinedOutput, "system", now);
    db.prepare("INSERT INTO events (taskId, type, payload, createdAt) VALUES (?, ?, ?, ?)").run(taskId, "CI_RAN", JSON.stringify({ preset, ok: allOk, summary }), now);
